...
def make_animals(L1, L2):
    ...
    for a in range(len(L1)):
        animals += [Animal(age=L1[a])]
        animals[a].set_name(L2[a])
...
class Employee(Person):
    ...
    def past_salary_freq(self):
        ...
        salary_hash = {}  # salary value:instances of
        salary_list = self.past_salaries_list()

        for i, sal in enumerate(salary_list):
            if sal in salary_hash:
                salary_hash[sal] += 1
            else: 
                salary_hash[sal] = 1
...
def counts(L):
    ...
    for p in L:
        if isinstance(p, Employee):
            ...
            pCount +=1
        nameHash.add(p.name)
    return (pCount, eCount, len(nameHash))
